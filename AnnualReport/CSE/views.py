...
from CSE.models import PHD, BTechStudentData, ResearchPaperMain, ResearchPaperUser, Teacher, BookPublished, Patents, \
...
def isAuthenticatedDepartment(request):
    if request.user.groups.filter(name="CSE").exists():
        return True
    else:
        return False
...
def teacherHome(request):
    if isAuthenticatedDepartment(request):
        return render(request, "teacherHome.html", {'depart': "CSE"})
...
def editTeacherProfile(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("CSE:teacherProfile")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if ResearchPaperMain.objects.filter(doi=doi).exists():
                ...
                return redirect("CSE:addResearchPapers")
            ...
            return redirect("CSE:researchPapers")
...
def addPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                phdData = PHD(user=request.user, department='Computer Science and Engineering', guide_name=guide_name, faculty_name=name,
            ...
            return redirect("CSE:phdDetails")
...
def studentData(request):
    if isAuthenticatedDepartment(request):
        ...
        allStudent = BTechStudentData.objects.filter(academic_year__gte=academic_year, academic_year__lte=int(toYear)-1)
...
def bookPublished(request):
    if isAuthenticatedDepartment(request):
        ...
        args = {
            "allPublishedBooks": allPublishedBooks
        }
...
def addBookPublished(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            bookPublish.save()
            return redirect("CSE:bookPublished")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if Patents.objects.filter(patent_no=patent_no).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("CSE:addPatents")
...
def addFacultyCompletedCourse(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("CSE:facultyCompletedCourse")
...
def guideAddPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            guidePhdData.save()
            return redirect("CSE:guidePhdDetails")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if GoogleScholarCitationDetails.objects.filter(user=request.user).filter(academic_year=academic_year).exists():
                ...
                return redirect("CSE:addGoogleScholarCitationDetails")
            ...
            return redirect("CSE:googleScholarCitationDetails")
...
def addguestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           return redirect("CSE:guestLecOrganizedOrDelivered")
...
def addeventConductedOrAttended(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           eventDetails.save()
           return redirect("CSE:eventConductedOrAttended")
