...
from Civil.models import PHD, BTechStudentData, ResearchPaperMain, ResearchPaperUser, Teacher, BookPublished, Patents, \
...
def isAuthenticatedDepartment(request):
    if request.user.groups.filter(name="Civil").exists():
        return True
    else:
        return False
...
def teacherHome(request):
    if isAuthenticatedDepartment(request):
        return render(request, "teacherHome.html", {'depart': "Civil"})
...
def editTeacherProfile(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Civil:teacherProfile")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if ResearchPaperMain.objects.filter(doi=doi).exists():
                ...
                return redirect("Civil:addResearchPapers")
            ...
            return redirect("Civil:researchPapers")
...
def addPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                phdData = PHD(user=request.user, department='Civil', guide_name=guide_name, faculty_name=name,
            ...
            return redirect("Civil:phdDetails")
...
def addBookPublished(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            bookPublish.save()
            return redirect("Civil:bookPublished")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if Patents.objects.filter(patent_no=patent_no).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("Civil:addPatents")
...
def addFacultyCompletedCourse(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Civil:facultyCompletedCourse")
...
def guideAddPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            guidePhdData.save()
            return redirect("Civil:guidePhdDetails")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if GoogleScholarCitationDetails.objects.filter(user=request.user).filter(academic_year=academic_year).exists():
                ...
                return redirect("Civil:addGoogleScholarCitationDetails")
            ...
            return redirect("Civil:googleScholarCitationDetails")
...
def guestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        ...
        args = {
            'allguestLecDetails': allguestLecDetails
            }
...
def addguestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Civil:guestLecOrganizedOrDelivered")
...
def addeventConductedOrAttended(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            eventDetails.save()
            return redirect("Civil:eventConductedOrAttended")
