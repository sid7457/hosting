...
from Electrical.models import PHD, BTechStudentData, ResearchPaperMain, ResearchPaperUser, Teacher, BookPublished, \
...
def isAuthenticatedDepartment(request):
    if request.user.groups.filter(name="Electrical").exists():
        return True
    else:
        return False
...
def teacherHome(request):
    if isAuthenticatedDepartment(request):
        return render(request, "teacherHome.html", {'depart': "Electrical"})
...
def editTeacherProfile(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Electrical:teacherProfile")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if ResearchPaperMain.objects.filter(doi=doi).exists():
                ...
                return redirect("Electrical:addResearchPapers")
            ...
            return redirect("Electrical:researchPapers")
...
def addPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                phdData = PHD(user=request.user, department='Electrical', guide_name=guide_name, faculty_name=name,
            ...
            return redirect("Electrical:phdDetails")
...
def addBookPublished(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            bookPublish.save()
            return redirect("Electrical:bookPublished")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if Patents.objects.filter(patent_no=patent_no).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("Electrical:addPatents")
            ...
            patent.save()
            return redirect("Electrical:patents")
...
def addFacultyCompletedCourse(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Electrical:facultyCompletedCourse")
...
def guideAddPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            guidePhdData.save()
            return redirect("Electrical:guidePhdDetails")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if GoogleScholarCitationDetails.objects.filter(user=request.user).filter(academic_year=academic_year).exists():
                ...
                return redirect("Electrical:addGoogleScholarCitationDetails")
            ...
            return redirect("Electrical:googleScholarCitationDetails")
...
def addguestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           return redirect("Electrical:guestLecOrganizedOrDelivered")
...
def addeventConductedOrAttended(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           eventDetails.save()
           return redirect("Electrical:eventConductedOrAttended")
