...
from Electronics.models import PHD, BTechStudentData, ResearchPaperMain, ResearchPaperUser, Teacher, BookPublished, \
    Patents, FacultyCompletedCourse, GuideforPHD, GoogleScholarCitationDetails, GuestLecOrganizedOrDelivered, EventConductedOrAttended


def isAuthenticatedDepartment(request):
    if request.user.groups.filter(name="Electronics").exists():
        return True
    else:
        return False
...
def teacherHome(request):
    if isAuthenticatedDepartment(request):
        return render(request, "teacherHome.html", {'depart': "Electronics"})
...
def editTeacherProfile(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Electronics:teacherProfile")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if ResearchPaperMain.objects.filter(doi=doi).exists():
                ...
                return redirect("Electronics:addResearchPapers")
            ...
            return redirect("Electronics:researchPapers")
...
def addPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                phdData = PHD(user=request.user, department='Electronics', guide_name=guide_name, faculty_name=name,
            ...
            return redirect("Electronics:phdDetails")
...
def addBookPublished(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            bookPublish.save()
            return redirect("Electronics:bookPublished")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if Patents.objects.filter(patent_no=patent_no).exists():
                messages.info(request, "Record Already Exist!")
                return redirect("Electronics:addPatents")
            ...
            patent.save()
            return redirect("Electronics:patents")
...
def addFacultyCompletedCourse(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("Electronics:facultyCompletedCourse")
...
def guideAddPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            guidePhdData.save()
            return redirect("Electronics:guidePhdDetails")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if GoogleScholarCitationDetails.objects.filter(user=request.user).filter(academic_year=academic_year).exists():
                ...
                return redirect("Electronics:addGoogleScholarCitationDetails")
            ...
            return redirect("Electronics:googleScholarCitationDetails")
...
def addguestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           return redirect("Electronics:guestLecOrganizedOrDelivered")
...
def addeventConductedOrAttended(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           eventDetails.save()
           return redirect("Electronics:eventConductedOrAttended")
