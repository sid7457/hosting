...
from ME.models import PHD, BTechStudentData, ResearchPaperMain, ResearchPaperUser, Teacher, BookPublished, Patents, \
...
def isAuthenticatedDepartment(request):
    if request.user.groups.filter(name="ME").exists():
        return True
    else:
        return False
...
def teacherHome(request):
    if isAuthenticatedDepartment(request):
        return render(request, "teacherHome.html", {'depart': "ME"})
...
def editTeacherProfile(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("ME:teacherProfile")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if ResearchPaperMain.objects.filter(doi=doi).exists():
                ...
                return redirect("ME:addResearchPapers")
            ...
            return redirect("ME:researchPapers")
...
def addPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if status == "Completed":
                phdData = PHD(user=request.user, department='Mechanical', guide_name=guide_name, faculty_name=name,
...
def addStudentData(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("ME:studentData")
...
def addBookPublished(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            bookPublish.save()
            return redirect("ME:bookPublished")
...
def addFacultyCompletedCourse(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            return redirect("ME:facultyCompletedCourse")
...
def guideAddPHD(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            guidePhdData.save()
            return redirect("ME:guidePhdDetails")
    ...
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
            ...
            if GoogleScholarCitationDetails.objects.filter(user=request.user).filter(academic_year=academic_year).exists():
                ...
                return redirect("ME:addGoogleScholarCitationDetails")
            ...
            return redirect("ME:googleScholarCitationDetails")
...
def addguestLecOrganizedOrDelivered(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           return redirect("ME:guestLecOrganizedOrDelivered")
...
def addeventConductedOrAttended(request):
    if isAuthenticatedDepartment(request):
        if request.method == 'POST':
           ...
           eventDetails.save()
           return redirect("ME:eventConductedOrAttended")
